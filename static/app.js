...
const techniciansTable = $('#technicians');
...
function colorBytechnician(technician) {
    return technician === null ? null : pickColor('technician' + technician.id);
}
...
function homeLocationPopupContent(technician) {
    return `<h5>technician ${technician.id}</h5>
Home Location`;
}
...
function getHomeLocationMarker(technician) {
    let marker = homeLocationMarkerByIdMap.get(technician.id);
    if (marker) {
        return marker;
    }
    marker = L.circleMarker(technician.homeLocation, { color: colorBytechnician(technician), fillOpacity: 0.8 });
    marker.addTo(homeLocationGroup).bindPopup();
    homeLocationMarkerByIdMap.set(technician.id, marker);
    return marker;
}
...
function renderRoutes(solution) {
    ...
    // technicians
    techniciansTable.children().remove();
    solution.technicians.forEach(function (technician) {
        getHomeLocationMarker(technician).setPopupContent(homeLocationPopupContent(technician));
        const {id, capacity, totalDemand, totalDrivingTimeSeconds} = technician;
        const percentage = totalDemand / capacity * 100;
        const color = colorBytechnician(technician);
        techniciansTable.append(`
      <tr>
        <td>
    ...
    for (let technician of solution.technicians) {
        const homeLocation = technician.homeLocation;
        const locations = technician.visits.map(visitId => visitByIdMap.get(visitId).location);
        L.polyline([homeLocation, ...locations, homeLocation], {color: colorBytechnician(technician)}).addTo(routeGroup);
    }
