...
class Visit(JsonDomainBase):
    ...
    demand: int
    technician: Annotated[Optional['Technician'],
                       InverseRelationShadowVariable(source_variable_name='visits'),
                       IdSerializer, TechnicianValidator, Field(default=None)]
    ...
    def driving_time_seconds_from_previous_standstill(self) -> int:
        ...
        if self.previous_visit is None:
            return self.technician.home_location.driving_time_to(self.location)
        else:
...
@planning_entity
class Technician(JsonDomainBase):
    id: Annotated[str, PlanningId]
    capacity: int
...
@planning_solution
class TechnicianRoutePlan(JsonDomainBase):
    name: str
    ...
    technicians: Annotated[list[Technician], PlanningEntityCollectionProperty]
    ...
    def total_driving_time_seconds(self) -> int:
        ...
        for technician in self.technicians:
            out += technician.total_driving_time_seconds
        return out

    def __str__(self):
        return f'TechnicianRoutePlan(name={self.id}, technicians={self.technicians}, visits={self.visits})'
